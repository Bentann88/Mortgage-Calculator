...
def amortization_shared(
    ...
    * *monthly_contrib* and *downpayments* must be the same length (one per borrower).
    ...
    # (scheduled payment is kept for reference but not used directly in this custom amortization)
    _ = (
        loan_amount
    ...
    while balance > 1e-6:  # iterate until the mortgage is cleared
        pmt_no += 1
        interest_due = balance * m_rate
        contrib_sum = sum(monthly_contrib)

        if contrib_sum < interest_due:
        ...
        # Final month: trim to keep balance from going negative
        ...
        rows.append(
            [pmt_no, round(principal_paid, rounding), round(interest_due, rounding), round(balance, rounding)]
...
with st.sidebar:
    ...
    # keep all numeric args *integer* (value, step, min_value) to avoid StreamlitMixedNumericTypesError
    loan = st.number_input("Loan amount ($)", value=362_316, step=1_000, min_value=1)
    ...
    contrib_a = st.number_input("Borrower A monthly contribution ($)", value=700, step=50, min_value=0)
    contrib_b = st.number_input("Borrower B monthly contribution ($)", value=2100, step=50, min_value=0)
    down_a = st.number_input("Borrower A down-payment ($)", value=13_000, step=1_000, min_value=0)
    down_b = st.number_input("Borrower B down-payment ($)", value=0, step=1_000, min_value=0)
...
if clicked:
    ...
    k1.metric("Months to payoff", f"{months_to_payoff}", help=f"≈ {months_to_payoff/12:.2f} years")
    k2.metric("Total interest paid", f"${schedule_df['Interest Paid'].sum():,.2f}")

    last = yearly_df.iloc[-1]
    share_cols = [c for c in yearly_df.columns if c.startswith("Share")]
    share_a, share_b = last[share_cols[0]], last[share_cols[1]]
